...
export default function CardFlipModal({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-10 ${SHELL_WIDTH} pt-6 pb-4 flex items-center justify-between`}
      >
      ...
      <div
        className={`relative z-10 flex-1 ${SHELL_WIDTH} pb-4 overflow-y-auto`}
      >
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {cards.map((card, index) => (
            <button
              ...
              className={`flip-card aspect-[3/4] text-left ${card.isFlipped ? "cursor-default" : "cursor-pointer transition-transform duration-100 active:scale-95"}`}
              onClick={
                card.isFlipped ? undefined : () => onCardClick(card.cardIndex)
              }
              style={{
            ...
            >
              <div
                className={`flip-card-inner ${card.isFlipped ? "flipped" : ""}`}
              >
      ...
      <div
        className={`relative z-10 ${SHELL_WIDTH} py-4 flex justify-center gap-3`}
      >
        {allFlipped && (
